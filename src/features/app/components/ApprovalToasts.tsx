...
export function ApprovalToasts({
  ...
  const renderParamValue = (value: unknown) => {
    if (value === null || value === undefined) {
      return { text: t("none"), isCode: false };
    }
        ...
        return (
          ...
          >
            <ToastHeader className="approval-toast-header">
              <ToastTitle className="approval-toast-title">{t("approvalNeeded")}</ToastTitle>
              {workspaceName ? (
            ...
            <ToastActions className="approval-toast-actions">
              ...
              {commandInfo && onRemember ? (
                <button
                  ...
                  title={t("allowCommandsStarting", { command: commandInfo.preview })}
                >
                  {t("alwaysAllow")}
                </button>
              ) : null}
