  ...
  return (
    ...
    >
      ...
      <SettingsToggleRow
        ...
        subtitle={
          <>
            在 config.toml 中写入 <code>features.multi_agent</code>。
          </>
        }
      >
        <SettingsToggleSwitch
      ...
      <SettingsToggleRow
        ...
        subtitle={
          <>
            最大打开的代理线程数。有效范围：<code>1-12</code>。更改立即保存。
          </>
        }
      >
        <div className="settings-agents-stepper" role="group" aria-label="最大代理线程数">
          <button
            type="button"
      ...
      <SettingsToggleRow
        ...
        subtitle={
          <>
            最大嵌套生成深度。有效范围：<code>1-4</code>。更改立即保存。
          </>
        }
      >
        <div className="settings-agents-stepper" role="group" aria-label="最大代理深度">
          <button
            type="button"
            className="ghost settings-agents-stepper-button"
            onClick={() => {
      ...
      <SettingsSubsection
        ...
        subtitle={
          <>
            在 <code>[agents.&lt;name&gt;]</code> 下添加自定义角色并创建其配置文件。
          </>
        }
      />
      <div className="settings-field settings-agents-form">
        ...
        {modelOptions.length === 0 && (
          <div className="settings-help">
            {modelOptionsLoading
              ? "正在加载工作区模型元数据，暂时使用默认备用模型。"
              : "在获取工作区模型元数据之前使用默认备用模型。"}
          </div>
        )}
      ...
      <SettingsSubsection
        title="已配置的代理"
        subtitle="管理自定义角色及其每个代理的配置文件。"
      />

      {settings && settings.agents.length === 0 && !isLoading && (
        <div className="settings-help">尚未配置自定义代理。</div>
      )}
      ...
      {isLoading && <div className="settings-help">加载代理设置中...</div>}
