...
export function FilePreviewPopover({
  ...
  const selectionLabel = selection
    ? t("linesLabel", { start: selection.start + 1, end: selection.end + 1 })
    : isImagePreview
      ? t("imagePreview")
      : t("noSelection");
    ...
    <PopoverSurface className="file-preview-popover" style={style}>
      <div className="file-preview-header">
        <div className="file-preview-title">
          ...
          {truncated && (
            <span className="file-preview-warning">{t("truncated")}</span>
          )}
        </div>
        <button
          type="button"
      ...
      {isLoading ? (
        <div className="file-preview-status">{t("loadingFile")}</div>
      ) : error ? (
