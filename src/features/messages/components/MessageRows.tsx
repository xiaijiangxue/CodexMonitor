    ...
    <>
      {isThinking && (
        <div className="working">
          ...
          <span className="working-text">{reasoningLabel || t("working")}</span>
        </div>
      )}
        ...
        <div className="turn-complete" aria-live="polite">
          ...
          <span className="turn-complete-label">
            {showPollingFetchStatus
              ? t("fetchingIn", { seconds: pollCountdownSeconds })
              : t("doneIn", { duration: formatDurationMs(lastDurationMs) })}
          </span>
          <span className="turn-complete-line" aria-hidden />
        </div>
      )}
  ...
  const imageItems = useMemo(() => {
    ...
    return item.images
      .map((image, index) => {
        ...
        return { src, label: t("imageNumber", { n: index + 1 }) };
      })
...
export const ReviewRow = memo(function ReviewRow({
  ...
  const { t } = useTranslation("messages");
  const title = item.state === "started" ? t("reviewStarted") : t("reviewCompleted");
  return (
...
export const UserInputRow = memo(function UserInputRow({
  ...
  const previewQuestion =
    first?.question?.trim() || first?.header?.trim() || t("inputRequested");
  const firstAnswer = first?.answers[0]?.trim() || t("noAnswer");
  const previewAnswer =
  ...
  return (
    <div className={`tool-inline user-input-inline ${isExpanded ? "tool-inline-expanded" : ""}`}>
      <button
        ...
        onClick={() => onToggle(item.id)}
        aria-expanded={isExpanded}
        aria-label={t("toggleAnswerDetails")}
      />
      <div className="tool-inline-content">
        <button
          type="button"
        ...
        >
          ...
          <span className="tool-inline-label">{t("answered")}</span>
          <span className="tool-inline-value user-input-inline-preview">
            {previewQuestion}: {previewAnswer}
            {extraQuestions > 0 ? ` ${t("moreCount", { count: extraQuestions })}` : ""}
          </span>
        </button>
        {isExpanded && (
          <div className="user-input-inline-details">
            {item.questions.map((question, index) => {
              const title = question.question || question.header || t("questionNumber", { n: index + 1 });
              return (
                <div
...
export const ToolRow = memo(function ToolRow({
  ...
  const summaryLabel = isFileChange
    ? t("filesEdited", { count: changeNames.length })
    : isCommand
      ? ""
      : summary.label;
  ...
  const handlePlanExport = useCallback(
    async (event: MouseEvent<HTMLButtonElement>) => {
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : t("unableToExportPlan");
        pushErrorToast({
          title: t("exportPlanFailed"),
          message,
        });
      } finally {
  ...
  return (
    <div className={`tool-inline tool-inline-row ${isExpanded ? "tool-inline-expanded" : ""}`}>
      <button
        ...
        onClick={() => onToggle(item.id)}
        aria-expanded={isExpanded}
        aria-label={t("toggleToolDetails")}
      />
      <div className="tool-inline-content">
        <button
          type="button"
        ...
        {showToolOutput && isPlan && (summary.output ?? "").trim() && (
          <div className="tool-inline-actions">
            ...
            >
              {isExportingPlan ? t("exporting") : t("exportMd")}
            </button>
          </div>
        )}
      </div>
...
export const ExploreRow = memo(function ExploreRow({ item }: ExploreRowProps) {
  const { t } = useTranslation("messages");
  const title = item.status === "exploring" ? t("exploring") : t("explored");
  return (
